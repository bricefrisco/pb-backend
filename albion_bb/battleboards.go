...
func (b *Battleboards) mapKills(battleId int, kills []KillsResponse) ([]*core.Record, error) {
	...
	for _, kill := range kills {
		...
		record.Set("killFame", kill.TotalVictimKillFame)

		record.Set("killerId", kill.Killer.Id)
		record.Set("killerName", kill.Killer.Name)
		record.Set("killerNameLower", strings.ToLower(kill.Killer.Name))
		record.Set("killerAlliance", kill.Killer.AllianceName)
		record.Set("killerAllianceLower", strings.ToLower(kill.Killer.AllianceName))
		record.Set("killerGuild", kill.Killer.GuildName)
		record.Set("killerGuildLower", strings.ToLower(kill.Killer.GuildName))
		...
		record.Set("victimNameLower", strings.ToLower(kill.Victim.Name))
		record.Set("victimAlliance", kill.Victim.AllianceName)
		record.Set("victimAllianceLower", strings.ToLower(kill.Victim.AllianceName))
		record.Set("victimGuild", kill.Victim.GuildName)
		record.Set("victimGuildLower", strings.ToLower(kill.Victim.GuildName))
